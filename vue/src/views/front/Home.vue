<template>
  <div style="min-height: calc(100vh - 86px);">
    <div style="width: 70%; margin: 10px auto">
      ...
      <div style="display: flex; gap: 10px">
        ...
        <div style="flex: 1">
          <div style="display: flex; border-bottom: 1px solid #ddd; margin-bottom: 20px">
            <div @click="loadTopNews('new')" class="top-news" :class="{ 'top-active' : sort === 'new' }">Latest News</div>
            <div @click="loadTopNews('hot')" class="top-news" :class="{ 'top-active' : sort === 'hot' }">Hot News</div>
          </div>

          <div style="padding: 0 10px">
            <div @click="$router.push('/front/newsDetail?id=' + item.id)" v-for="item in topNews" :key="item.id" style="display: flex; gap: 10px; margin: 10px 0; cursor: pointer">
      ...
      <!-- Bottom area -->
      <div style="margin: 30px 0; display: flex; gap: 20px; min-height: 400px">
        <!-- Left side -->
        <div style="flex: 1">
          <div style="display: flex; margin-bottom: 10px;">
            <div @click="loadCategoryNews(null)" class="category-item" :class="{ 'category-active' : category === null }">All</div>
        ...
        <div style="width: 260px">
          <div style="margin-bottom: 10px; display: flex; align-items: center">
            <div style="flex: 1; font-size: 24px;">Public Activities</div>
            <a href="/front/activity">More >></a>
          </div>
          <div class="card">
            <div @click="$router.push('/front/activityDetail?id=' + item.id)" style="display: flex; gap: 10px; margin-bottom: 15px; cursor: pointer" v-for="item in activityList" :key="item.id">
